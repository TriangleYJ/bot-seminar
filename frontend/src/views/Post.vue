...
export default {
  ...
  methods: {
    async addNewComment (comment) {
      ...
      const parent_comment = this.post.comments.find(comm => comm.id === comment.parent_comment)

      // TODO : alert all on one re-reply
      console.log(comment.parent_article ? this.post.created_by.id : parent_comment.created_by.id)
      axios.post('http://localhost:3000/bot/alert/' + (comment.parent_article ? this.post.created_by.id : parent_comment.created_by.id), {
        sender: comment.created_by.profile.nickname,
        type: 'reply',
        reply_type : (comment.parent_article ? '' : 're'),
        content: comment.content,
        post_id: this.post.id,
        target_id: comment.id,
        my_content: (comment.parent_article ? this.post.title : parent_comment.content)
    ...
    async refresh () {
      await console.log('fetch', this.post)
